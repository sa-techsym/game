...
public class Tube extends CylinderElement {
	public Tube(float start_angle, float end_angle, float radius, float height) {
		_slices = (int) ((end_angle - start_angle) / _DEGREES_PER_SLICE);
		...
		float[] coords = new float[2 * slices * _COORDS_PER_VERTEX];
		for (int i = 0; i < slices; i++) {
			int offset = 2 * _COORDS_PER_VERTEX * i;
			coords[offset + 0] = coords[offset + 3] = (float) Math.cos(start_angle + i * degrees?per?slice);
			coords[offset + 1] = coords[offset + 4] = (float) Math.sin(start_angle + i * _DEGREES_PER_SLICE);
